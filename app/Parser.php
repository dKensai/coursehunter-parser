...
class Parser{
   ...
   */    
    private function parseMainInfo($html){
            ...
            preg_match('/class="hero-title">(.+?)</i', $html, $match);

            $this->main_info['rus_name'] = isset($match[1]) ? $this->sanitize_string($match[1]) : $this->rus_name_error;

            // original name
            preg_match('/class="hero-description">(.+?)</i', $html, $match);
   ...
   */    
    private function getSampleFiles($html){
          ...
          $dir = Config::BASE_DIR . $this->main_info['rus_name'];
            ...
            preg_match('/href="(.*?)".*?(?=Download.*?course.*?materials)/ui', $html, $match);

            $zip = isset( $match[1] ) ? $match[1] : false;
   ...
   */
    private function getVideoFiles($html){
        ...
        $dir = Config::BASE_DIR . $this->main_info['rus_name'];
            ...
            preg_match_all('/myPlaylist\.push\({\n*.*?"file":.*"(.+?)".*?\n?.*\n?.*?\n*.*\n?.*"title":.*?"(.*?)"/ui', $html, $match);

            $lessons_list_names = isset($match[2]) ? $match[2] : $this->lessons_list_names_error;
            $lessons_list_videos = isset($match[1]) ? $match[1] : $this->lessons_list_names_error; 
        ...
        if( is_array( $lessons_list_names ) ){
            ...
            foreach ($lessons_list_names as $k => $v) {
               ...
               if( file_put_contents( $dir . "/" . $lesson_num . ". " . $file_name . ".mp4", file_get_contents($lessons_list_videos[$k])) ){
